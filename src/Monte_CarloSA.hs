module Monte_CarloSA(traveling_monte_carloSA) where
...
traveling_monte_carloSA :: [[Float]] -> IO (Float,[Int])
traveling_monte_carloSA [] = return (0,[])
traveling_monte_carloSA (hL:t) = do
                                let n = length hL
                                ...
                                retValue <- iterations (hL:t) n route dist 100 100 1
                                return retValue
...
-- iteração
iterations :: [[Float]] -> Int -> [Int] -> Float -> Int -> Int -> Float -> IO (Float,[Int])
iterations _ _ route dist 0 _ _ = return (dist,route)
iterations ll n route dist it itStart t = do
                                c <- randomRIO (0,n-1)
                                let (prev,n1,n2) = getNeighbors c (n-1)
                                let delta = calcDelta (c,prev,n1,n2) route ll
                                threshold <- randomRIO (0.0,1.0)
                                let b = (delta < 0 || (t > 0.001 && (exp (-delta/t) >= threshold)))
                                let routeUp = swap route b (c,n1)
                                let distUp = updateDist dist delta b
                                let itUp = incr it b itStart
                                let tUp = t * 0.999
                                retValue <- iterations ll n routeUp distUp itUp itStart tUp
                                return retValue
...
updateDist :: Float -> Float -> Bool -> Float
updateDist dist delta b
    | b = dist + delta
    | otherwise = dist
